...
export function registerCloseHandler(composer: Composer<BotContext>) {
  composer.action(/^pos_close:(.+):(\d+)$/, async (ctx) => {
    ...
    const positionsResponse = await UniversalApiService.getPositions(ctx.session.userId!.toString(), activeExchange);
    const position = (positionsResponse.data as any[]).find(p => p.symbol === symbol && parseFloat(p.size) !== 0);

    const closeSide = position && parseFloat(position.size) < 0 ? 'BUY' : 'SELL';
