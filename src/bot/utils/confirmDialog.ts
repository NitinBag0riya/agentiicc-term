 ...
 */
function formatOperationDetails(op: TradingOperation): string {
  const lines: string[] = [];
  ...
  switch (op.operation) {
    case 'CREATE_ORDER':
      ...
      lines.push(`Type: ${op.params.type}`);

      lines.push(``);
      lines.push(`📝 **Input**`);
      if (op.params.quantity) {
        lines.push(`Quantity: ${op.params.quantity}`);
      } else if (op.params.quantityInUSD) {
        lines.push(`Amount: $${op.params.quantityInUSD} USDT`);
      }

      if (op.params.price) lines.push(`Price: $${op.params.price}`);
      if (op.params.stopPrice) lines.push(`Stop: $${op.params.stopPrice}`);

      const settings: string[] = [];
      if (op.params.leverage) settings.push(`Leverage: ${op.params.leverage}x`);
      if (op.params.marginMode) settings.push(`Margin: ${op.params.marginMode}`);
    ...
    case 'CLOSE_POSITION':
      lines.push(`Symbol: ${op.params.symbol}`);
      lines.push(`Close: ${op.params.percentage || 100}%`);
      break;
    
    default:
      lines.push(`Operation: ${op.operation}`);
      lines.push(`Params: ${JSON.stringify(op.params)}`);
  }
...
export async function handleRecalc(
  ...
  try {
    ...
    const op = stored.operation as TradingOperation;
    ...
    try {
      ...
      const recalcOp: TradingOperation = JSON.parse(JSON.stringify(op)); // Deep clone
