...
export class AsterAdapter implements ExchangeAdapter {
  ...
  async cancelAllOrders(symbol?: string): Promise<{ success: boolean; canceledCount: number; message: string }> {
    ...
    try {
        console.log('[AsterAdapter] Canceling all orders for:', symbol);
        const data: any = await this.request('/fapi/v1/allOpenOrders', { symbol }, 'DELETE');
        console.log('[AsterAdapter] Cancel all response:', JSON.stringify(data));
        
        // Check if the response indicates success
        const isSuccess = data?.code === 200 || data?.msg === 'success' || !data?.code;
        
        return {
            success: isSuccess,
            canceledCount: Array.isArray(data) ? data.length : -1,
            message: isSuccess ? 'All open orders canceled successfully' : `Cancel failed: ${JSON.stringify(data)}`
        };
    } catch (error: any) {
        console.error('[AsterAdapter] Cancel all orders error:', error.message);
        return {
            success: false,
            canceledCount: 0,
  ...
  async getPositions(symbol?: string): Promise<Position[]> {
    try {
      ...
      return positions
        ...
        .map((p: any) => ({
          // Return raw Binance fields for compatibility with existing UI
          symbol: p.symbol,
          ...
          // Also include normalized fields for adapter interface
          size: p.positionAmt,
          unrealizedPnl: p.unRealizedProfit,
          side: parseFloat(p.positionAmt) > 0 ? 'LONG' : 'SHORT',
        }));
