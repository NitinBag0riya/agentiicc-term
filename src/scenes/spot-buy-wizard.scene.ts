/**
 * Spot Buy Wizard Scene
 *
 * Simple flow for buying spot assets with USDT
...
export const spotBuyWizard = new Scenes.WizardScene<BotWizardContext>(
  ...
  async (ctx) => {
    ...
    if (parsed.type === 'USD') {
      ...
      // For spot BUY, this is USDT amount (quoteOrderQty)
      quantityParams.quantityInUSD = parsed.value.toString();
    }
...
// ========== Leave Handler ====================
spotBuyWizard.leave(async (ctx) => {
  console.log('[SpotBuyWizard] Exited');
  if (ctx.wizard) {
    ctx.wizard.state = {};
  }
});
