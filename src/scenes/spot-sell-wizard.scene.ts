...
export const spotSellWizard = new Scenes.WizardScene<BotWizardContext>(
  'spot-sell-wizard',
  ...
  async (ctx) => {
    ...
    const operation: AsterWriteOp = {
      ...
      metadata: {
        action: `Sell ${input} ${state.asset} on ${ctx.session.activeExchange ? ctx.session.activeExchange.toUpperCase() : 'ASTER'}`,
