...
export async function initSchema(): Promise<void> {
  ...
  await query(`CREATE INDEX IF NOT EXISTS idx_users_is_verified ON users(is_verified)`);
  await query(`CREATE INDEX IF NOT EXISTS idx_referrals_referrer_user_id ON referrals(referrer_user_id)`);
  await query(`CREATE INDEX IF NOT EXISTS idx_referrals_referred_user_id ON referrals(referred_user_id)`);
  await query(`CREATE INDEX IF NOT EXISTS idx_orders_user_id ON orders(user_id)`);
  await query(`CREATE INDEX IF NOT EXISTS idx_orders_telegram_id ON orders(telegram_id)`);
  await query(`CREATE INDEX IF NOT EXISTS idx_orders_operation_id ON orders(operation_id)`);
  await query(`CREATE INDEX IF NOT EXISTS idx_orders_symbol ON orders(symbol)`);
  await query(`CREATE INDEX IF NOT EXISTS idx_orders_user_confirm ON orders(user_confirm)`);
  await query(`CREATE INDEX IF NOT EXISTS idx_orders_created_at ON orders(created_at)`);
  await query(`CREATE INDEX IF NOT EXISTS idx_orders_exchange_order_id ON orders(exchange_order_id)`);
  await query(`CREATE INDEX IF NOT EXISTS idx_webhook_logs_update_id ON webhook_logs(update_id)`);
  await query(`CREATE INDEX IF NOT EXISTS idx_webhook_logs_processed_at ON webhook_logs(processed_at)`);
  await query(`CREATE INDEX IF NOT EXISTS idx_webhook_logs_payload ON webhook_logs USING GIN (payload)`);
